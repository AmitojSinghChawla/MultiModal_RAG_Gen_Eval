...
def save_results(per_q, summary):
    json.dump({"summary": summary, "per_question": per_q},
              open(RESULTS_FILE, "w", encoding="utf-8"), indent=2, ensure_ascii=False)
...
def token_f1(pred, gt):
    ...
    if not common:
        return 0.0
    return round(2 * (common / len(p)) * (common / len(g)) /
                 ((common / len(p)) + (common / len(g))), 4)

def rouge_l(pred, gt):
    return round(rouge_lib.RougeScorer(["rougeL"], use_stemmer=True)
                 .score(gt, pred)["rougeL"].fmeasure, 4)

def string_metrics(pred, gt):
    return {"exact_match": exact_match(pred, gt),
...
def generate_answer(question, chunks):
    llm, content, ctx_texts = ChatOpenAI(model=MODEL, temperature=0, api_key=API_KEY), [], []

    for chunk in chunks:
        src, mod = chunk["source_pdf"], chunk["modality"]
        if mod == "image" and chunk.get("image_b64"):
            content += [{"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{chunk['image_b64']}", "detail": "high"}},
                        {"type": "text", "text": f"[Image from {src}]"}]
            ctx_texts.append(chunk["retrieval_text"])
        else:
            text = chunk.get("raw_text") or chunk["retrieval_text"]
            content.append({"type": "text", "text": f"[{mod.upper()} from {src}]\n{text}\n"})
...
def score_ragas(question, answer, contexts, ground_truth):
    ...
    return {k: safe_float(row.get(k)) for k in
...
def mean_std(vals):
    ...
    if not clean:
        return 0.0, 0.0
    return round(statistics.mean(clean), 4), round(statistics.stdev(clean) if len(clean) > 1 else 0.0, 4)

def compute_summary(entries):
    scored = [e for e in entries if ragas_done(e)]
    return {k: {"mean": m, "std": s} for k, (m, s) in {
        "exact_match":        mean_std([e["string_metrics"]["exact_match"]              for e in entries]),
        "token_f1":           mean_std([e["string_metrics"]["token_f1"]                 for e in entries]),
        "rouge_l":            mean_std([e["string_metrics"]["rouge_l"]                  for e in entries]),
        "answer_correctness": mean_std([e["ragas_metrics"]["answer_correctness"]         for e in scored]),
        "faithfulness":       mean_std([e["ragas_metrics"]["faithfulness"]               for e in scored]),
        "context_precision":  mean_std([e["ragas_metrics"]["context_precision"]          for e in scored]),
        "context_recall":     mean_std([e["ragas_metrics"]["context_recall"]             for e in scored]),
    }.items()}


# ── Summary table ─────────────────────────────────────────────
def print_summary(summary):
    keys = ["exact_match","token_f1","rouge_l","answer_correctness","faithfulness","context_precision","context_recall"]
    heads = ["Method","EM","F1","ROUGE-L","AnsCorr","Faithful","CtxPrec","CtxRecall"]
    w = 13
...
def main():
    ...
    for method in METHODS:
        ...
        for i, q in enumerate(questions, 1):
            qid, quest, gt = q.get("question_id", f"q{i}"), q["question"], q["ground_truth"]
            ...
            print(f"       EM={sm['exact_match']:.2f}  F1={sm['token_f1']:.4f}  ROUGE-L={sm['rouge_l']:.4f}")

            per_q[method].append({
                "question_id": qid, "question": quest, "ground_truth": gt,
                "prediction": answer, "context_texts": ctx_texts,
                "retrieved_ids": [c["chunk_id"] for c in chunks],
                "string_metrics": sm, "ragas_metrics": None,
            })
        ...
        for i, entry in enumerate(pending, 1):
            print(f"  [{i:02d}/{len(pending)}] {entry['question_id']}...", end=" ", flush=True)
            try:
                rs = score_ragas(entry["question"], entry["prediction"],
                                 entry.get("context_texts", []), entry["ground_truth"])
                entry["ragas_metrics"] = rs
                print(f"AC={rs['answer_correctness']}  F={rs['faithfulness']}  "
                      f"CP={rs['context_precision']}  CR={rs['context_recall']}")
            except Exception as e:
                print(f"FAILED: {e}")
                entry["ragas_metrics"] = {k: None for k in
                    ["answer_correctness","faithfulness","context_precision","context_recall"]}
